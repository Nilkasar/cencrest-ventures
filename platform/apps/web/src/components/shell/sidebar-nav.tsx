  ...
  return (
    <Link
      ...
      className={cn(
        ...
        active
          ? "bg-verdant-400/[0.12] text-ink-0"
          : "text-ink-0/55 hover:bg-ink-0/[0.06] hover:text-ink-0/90",
      )}
    >
      {active && (
...
export function SidebarNav({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <nav className="flex-1 overflow-y-auto px-3 py-4 flex flex-col gap-5">
      {navGroups.map((group) => (
        <div key={group.label}>
          <p className="px-3 mb-1.5 text-[10.5px] font-semibold uppercase tracking-[0.1em] text-ink-0/35 select-none">
            {group.label}
          </p>
